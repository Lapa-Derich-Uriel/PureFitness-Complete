...
namespace PureFitness.Controllers
{
    ...
    public class AdminController : Controller
    {
        ...
        {
            var admins = _context.Staffs
                .Include(s => s.User)
                .Where(s => s.User != null && s.User.Role!.RoleName == "Admin")
                .ToList();
        ...
        // --------------------------
        [HttpPost]
        public IActionResult AddAdmin(StaffViewModel StaffVM)
        {
        ...
        {
            var staff = _context.Staffs.Include(u => u.User).FirstOrDefault(i => i.StaffId == id);
        ...
        // --------------------------
        [HttpPost]
        public IActionResult EditAdminMember(StaffViewModel StaffVM)
        {
            ...
            if (staffFromDB.User != null)
            {
                ...
                // Only re-hash if user entered a new raw password
                if (!string.IsNullOrEmpty(StaffVM.user.Password) && StaffVM.user.Password.Length < 60)
                {
                    staffFromDB.User.Password = _passwordHasher.HashPassword(null!, StaffVM.user.Password);
                }
            }
        ...
        public async Task<IActionResult> Search(string lowersearch)
        {
            ...
            return PartialView("_AdminTableBody", result);
        }
